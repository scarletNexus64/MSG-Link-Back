...
@section('header', 'Modifier le cadeau')

@section('content')
<div class="max-w-3xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('admin.gift-management.update', $gift) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description"
                              id="description"
                              rows="3"
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-primary-500 focus:border-primary-500 @error('description') border-red-500 @enderror">{{ old('description', $gift->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <input type="color"
                               ...
                               id="background_color"
                               value="{{ old('background_color', $gift->background_color) }}"
                ...
                <div>
                    ...
                    <select name="gift_category_id"
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('gift_category_id', $gift->gift_category_id) == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
